...
test('completion receipt rejects malformed model output instead of throwing', () => {
  for (const bad of [undefined, null, {}, 'nope', { v: 1, status: 'complete' }, { v: 2, task: 't', status: 'complete', claims: [] }]) {
    const result = validateCompletionReceipt(bad, { changedFiles: [], verificationCommands: [] });
    assert.equal(result.valid, false);
    assert.ok(result.errors.length > 0);
  }
});

test('completion receipt requires captured evidence, not just a claimed exit code', () => {
  const result = validateCompletionReceipt({
    v: 1,
    task: 'add feature',
    status: 'complete',
    claims: [{ type: 'tests_pass', command: 'npm test', exitCode: 0 }],
  }, { changedFiles: [], verificationCommands: [] });

  assert.equal(result.valid, false);
  assert.match(result.errors.join('\n'), /No captured verification evidence/);
});
